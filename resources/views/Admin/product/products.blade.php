...
@push('css')
    <style>
        ...
        #productTable thead th.col-pinned {
            z-index: 7;
            background: #eef1f4;
        }
        ...
        .sort-btn .arrow-up path,
        .sort-btn .arrow-down path {
            stroke: currentColor;
        }

        .sort-btn.asc .arrow-up path {
            stroke: #1a1d20;
            stroke-width: 2.4;
        }

        .sort-btn.asc .arrow-down path {
            stroke: #c7cbd1;
        }

        .sort-btn.desc .arrow-down path {
            stroke: #1a1d20;
            stroke-width: 2.4;
        }

        .sort-btn.desc .arrow-up path {
            stroke: #c7cbd1;
        }
...
@section('content')
    <div class="card p-3">
        <div class="card-header toolbar d-flex align-items-center justify-content-end gap-2 mb-2 flex-wrap px-0">
            <select name="" id=""></select>
            <button id="resetBtn" class="btn btn-outline-secondary btn-sm">
                <i class="fa-solid fa-rotate-left"></i> Reset pins
            </button>
            <a href="{{ route('admin.products.form') }}" class="btn btn-primary btn-sm">
                <i class="fa-solid fa-box-open"></i> Add Product
            </a>
        </div>
        
        <div class="card-body px-0">
            <p class="hint text-muted small text-end" id="colCountHint"></p>
            @php
                $columns = [
            ...
            <div class="table-responsive" style="max-height:560px; overflow-y:auto;">
                <table class="table table-bordered table-striped table-hover align-middle mb-0" id="productTable">
                    <thead>
                        <tr>
                                    ...
                                    <div class="th-inner">
                                        <button type="button" class="pin-btn" title="Pin column">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                ...
                                                <path
                                                    d="M5 17h14l-1.6-2.1a3 3 0 0 1-.6-1.8V9a5 5 0 0 0-10 0v4.1a3 3 0 0 1-.6 1.8L5 17z">
                                                </path>
                                            </svg>
                                        </button>
                    ...
                    <tbody>
                        @php
                            ...
                            $dateBadge = function ($dateStr) {
                                ...
                                if ($date->eq($today)) {
                                    return ['label' => 'Today', 'class' => 'badge-today'];
                                }
                                if ($date->eq($today->copy()->subDay())) {
                                    return ['label' => 'Yesterday', 'class' => 'badge-yesterday'];
                                }
                                if ($date->isSameWeek($today)) {
                                    return ['label' => 'This wk', 'class' => 'badge-thisweek'];
                                }
                                if ($date->isSameWeek($today->copy()->subWeek())) {
                                    return ['label' => 'Last wk', 'class' => 'badge-lastweek'];
                                }
                                if ($date->isSameMonth($today)) {
                                    return ['label' => 'This mo', 'class' => 'badge-thismonth'];
                                }
    ...
    <script>
        $(function() {
            ...
            function setup() {
                $table.find('thead th').each(function(i) {
                    ...
                    $table.find('tbody tr').each(function() {
                        $(this).find('td').eq(i).attr('data-idx', i);
                    });
                });
            }
            ...
            function getColumnOrder() {
                var allIdx = $table.find('thead th').map(function() {
                    return +$(this).data('idx');
                }).get();
                var unpinned = allIdx
                    .filter(function(i) {
                        return pinned.indexOf(i) === -1;
                    })
                    .sort(function(a, b) {
                        return a - b;
                    });
            ...
            function sortByColumn(idx) {
                ...
                var rows = $table.find('tbody tr').get().sort(function(rowA, rowB) {
                    ...
                    var a = getSortValue(cellA),
                        b = getSortValue(cellB);
            ...
            // =====================================================================
            function bindEvents() {
                $table.on('click', '.pin-btn', function() {
                    togglePin($(this));
                });
